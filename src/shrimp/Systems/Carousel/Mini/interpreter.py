...
class MiniInterpreter:
    ...
    def _eval_sequence_elements(self, elements):
        ...
        for es in elements:
            ...
            tc_args.append((len(es) * weight, sequence(*[e[1] for e in es]).degrade_by(deg_ratio)))
    ...
    def eval_polymeter(self, node):
        ...
        fast_params = [TidalFraction(node["steps"], len(seq["elements"])) for seq in node["seqs"]]
        return stack(
            *[self.eval(seq).fast(fparam) for seq, fparam in zip(node["seqs"], fast_params)]
        )
    ...
    def eval_modifier(self, node):
        ...
        if node["op"] == "degrade":
            ...
            arg = node["value"]
            if arg["op"] == "count":
                return lambda w_p: [(w_p[0], w_p[1], TidalFraction(arg["value"], arg["value"] + 1))]
            elif arg["op"] == "value":
